...
import pandas as pd
import plotly.express as pxfrom __future__ import annotations
...
import streamlit as stfrom __future__ import annotations
...
def to_korean_course_name(course: str) -> str:
    return COURSE_NAME_KO.get(str(course), str(course))
...
def build_calendar_html(df_student: pd.DataFrame, target_week: int) -> str:
    ...
    for _, r in df_w.iterrows():
        ...
        end_txt = str(r.get("종료", ""))
        room_txt = str(r.get("강의실", "-"))
        text_main = f"<div class='course'>{show_course}</div><div>{start_txt}~{end_txt}</div><div>강의실 {room_txt}</div>"
        for s in range(s0, s1):
            tlabel = slot_to_time(s)
            if s == s0:
                table[tlabel][day] = text_main
...
if menu == "학번별 조회":
    ...
    cols = ["과목명", "학년", "주차", "요일", "날짜", "시작", "종료", "강의실", "변경상태"]
    st.dataframe(df_all[cols], use_container_width=True, hide_index=True)
    ...
    cols = [
        "과목명", "학년", "기존시간", "시작", "요일", "주차", "기존강의실", "강의실",
        "시간변경여부", "강의실변경여부", "변경상태",
    ]
